...
#    cu = cx.cursor()
    ...
    for webPageFilePath in webPageFilePathList:
        ...
        while True:
            start = wb.find('<dl class=\\"feed_list W_linecolor')
            if start == -1:
                break
            wbTemp = wb[start+3:]
        ...
        for item in dlList:
            ...
            line =  '%s,%s,%s,%s,%s,%s,%s,%s\n' % (nickname, title, praised, forward, favorite, comment, date, time)
            print line
...
def utf82gbk(hint):
    return hint.decode('utf-8').encode('gbk')
...
try:
    ...
    if write_type == REWRITE:
        ...
        r.write('昵称,头衔,赞,转发,收藏,评论,日期,时间\n')
